               ...
               {testimonials.map((t: any, i: number) => (
                  <div key={i} className="p-10 rounded-3xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-xl relative group hover:-translate-y-2 transition-all duration-300 hover:shadow-primary/20 hover:border-primary/30">
                     ...
                     <div className="flex items-center gap-4 mt-auto border-t border-zinc-100 dark:border-zinc-800 pt-6">
                        <div className="w-14 h-14 rounded-full overflow-hidden bg-primary text-primary-foreground flex items-center justify-center font-bold text-2xl shrink-0 shadow-md relative">
                           {t.avatar || t.src || t.image ? (
                              <Image
                                 src={(t.avatar || t.src || t.image) || ""}
                        ...
                        <div>
                           <h4 className="font-bold text-foreground text-lg">{t.name || "Anonymous"}</h4>
                           <p className="text-sm text-primary font-semibold">{t.role || "Verified User"}</p>
                        </div>
                     </div>
                  </div>
               ))}
